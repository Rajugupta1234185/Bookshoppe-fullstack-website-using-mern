...
const Dashboard = () => {
  ...
  const handleAddToCart = async() => {
    ...
    const response=await axios.post('http://localhost:5000/api/cart/add',{quantity:selectedQuantity,bookId:currentBook._id,token:token})
    ...
    <div className="home-maincontent">
      <div className="firstdiv">
        ...
        <div style={{ display: "flex" }}>
          <div className="cart" onClick={handlecart}><p>Cart</p></div>
          <div className="signin" onClick={profile}><p>Profile</p></div>
        </div>
      </div>
          ...
          {fiction.slice(fictionPage * booksPerPage, (fictionPage + 1) * booksPerPage).map((book, index) => (
            <div key={index} className="book-item">
              ...
              <div className="book-hover">
                ...
                <button className="add-cart" onClick={() => openQuantityModal(book)}>Add to Cart</button>
                <button className="buy-now">Buy Now</button>
                <button className="description" onClick={() => openDescriptionModal(book.description)}>Description</button>
              </div>
            </div>
          ))}
        </div>
